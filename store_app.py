...
if not st.session_state['logged_in']:
            ...
            if st.form_submit_button("SIGN IN"):
                if mode == "Admin" and p == ADMIN_PASSWORD:
                    ...
                    if any(str(r['Username']).strip()==u.strip() and str(r['PIN']).strip()==p.strip() for r in recs):
                        perm_val = next(r['Permission'] for r in recs if str(r['Username']).strip()==u.strip())
                        st.session_state.update(logged_in=True, user=u, role="Technician", permission=perm_val)
                        st.rerun()
    ...
    if nav == "DASHBOARD":
        ...
        output.seek(0)
        st.download_button(label="Download Excel", data=output, file_name="assets.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        st.markdown('</div>', unsafe_allow_html=True)
