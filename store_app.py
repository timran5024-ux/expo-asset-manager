...
# ==========================================
# 1. WATERMARK BACKGROUND & PREMIUM CSS
# ==========================================
...
# Check if logo exists to create the background
bg_css = ""
if os.path.exists("logo.png"):
    try:
        ...
        /* Faded overlay to keep text readable */
        .stApp::before {{
            content: "";
            position: absolute;
...
<style>
    ...
    button[key="final_refresh_btn"] {{ background-color: #007BFF !important; color: white !important; font-weight: 800 !important; }}
    button[key="final_logout_btn"] {{ background-color: #FF4B4B !important; color: white !important; font-weight: 800 !important; }}
...
SESSION_SECRET = "expo_final_v137_master"
    ...
    if nav == "DASHBOARD":
        ...
        if not df.empty:
            ...
            for i, model in enumerate(models):
                ...
                with grid[i % 3]:
                    st.markdown(f"<div style='text-align:center;'><b style='font-size:18px;'>{model}</b><br><span style='font-size:12px; color:#555;'>Total: {t}</span><br><span style='font-size:11px; color:#28A745;'>Avail: {a}</span> | <span style='color:#007BFF;'>Issued: {s}</span> | <span style='color:#DC3545;'>Faulty: {f}</span></div>", unsafe_allow_html=True)
                    ...
                    st.plotly_chart(fig, use_container_width=True, key=f"d_v137_{i}")
        ...
        with t1:
            with st.form("f_add"):
                ...
                lo = c3.selectbox("Store", ["MOBILITY STORE-10", "MOBILITY STORE-8", "SUSTAINABILITY BASEMENT", "TERRA BASEMENT"], key="st_add")
        ...
        with t2:
            mod_sn = st.text_input("Serial Number to Modify")
            if mod_sn:
                match = df[df['SERIAL'].astype(str).str.upper() == mod_sn.strip().upper()]
                if not match.empty:
                    item = match.iloc[0]
                    with st.form("f_mod"):
                        c1, c2 = st.columns(2)
                        ...
                        n_lo = c2.selectbox("Store", ["MOBILITY STORE-10", "MOBILITY STORE-8", "SUSTAINABILITY BASEMENT", "TERRA BASEMENT"], key="st_mod")
                        if st.form_submit_button("UPDATE ASSET"):
                            ridx = int(df.index[df['SERIAL'] == mod_sn][0]) + 2
                            ws_inv.update(f"B{ridx}:C{ridx}", [[n_br, n_md]])
                            ws_inv.update_cell(ridx, 5, n_mc)
                            ws_inv.update_cell(ridx, 7, n_lo)
                            st.success("DONE"); time.sleep(1); sync(); st.rerun()

        with t3:
            rem_sn = st.text_input("Serial Number to Remove")
            if rem_sn:
                match = df[df['SERIAL'].astype(str).str.upper() == rem_sn.strip().upper()]
                if not match.empty:
                    st.error(f"Deleting {match.iloc[0]['MODEL']} (SN: {rem_sn})")
                    if st.button("CONFIRM DELETE"):
                        ws_inv.delete_rows(int(df.index[df['SERIAL'] == rem_sn][0]) + 2)
                        st.success("DONE"); time.sleep(1); sync(); st.rerun()
